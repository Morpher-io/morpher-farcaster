...
export function PortfolioSummary() {
  ...
  React.useEffect(() => {
    let profileImage =
      portfolio?.profile_base64 || portfolioContext?.profile_base64;
    if (context) {
      if (!profileImage) {
    ...
    <>
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p
            className="text-primary cursor-pointer text-2xl font-bold"
            onClick={() => route.push("/leaderboard")}
          >
            #
            {portfolio?.returns_rank && portfolio?.returns_rank > 0
