...
export function PortfolioStats() {
  const { portfolio, returns, loading, currencyList, positionList } =
    usePortfolioStore();
    ...
    <div className="grid grid-cols-3 gap-2">
      {stats.map((stat) => (
        <Card key={stat.title} className="p-2">
          <CardContent className="flex h-full flex-col items-center justify-between p-1 text-center">
            <p className="text-muted-foreground mb-1 text-xs leading-tight font-semibold">
              {stat.title}
            </p>
            <div className="">
              {stat.data ? (
                <>
                  <p
                    className={`mt-1 text-xs font-semibold ${stat.data.isPositive === false ? "text-secondary" : "text-[var(--dark)]"}`}
                  >
                    {stat.data.isPositive ? "+" : ""}${" "}
                    {usdFormatter(stat.data.valueUsd)}
                  ...
                  <p
                    className={`mt-1 text-xs ${stat.data.isPositive === false ? "text-secondary" : "text-[var(--dark)]"}`}
                  >
                    {stat.data.isPositive ? "+" : ""}
                    {tokenValueFormatter(stat.data.valueMph)} MPH
                  </p>
                </>
              ) : (
                <p className="text-baseline font-bold">{stat.value}</p>
              )}
            </div>
          </CardContent>
        </Card>
