    ...
    <div className="mt-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t("ORDER_EXECUTION")}</CardDescription>
        </CardHeader>
        <CardContent>
          {!marketData?.market_id?.includes("CRYPTO_") && (
            <p className="text-sm font-normal">{t("ORDER_MARKET_HOURS")}</p>
          )}
          ...
          {order && (
            <div className="mt-2">
              ...
              <p className="text-muted-foreground text-xs">
                <b>{t("PRICE")}</b>: ${" "}
                {tokenValueFormatter(Number(order.price || 0) / 10 ** 8)}
              </p>
              <p className="text-muted-foreground text-xs">
                <b>{t("DATE_ENTERED")}</b>:{" "}
                {order.created_at
                  ? new Date(Number(order.created_at)).toDateString()
                  : ""}
              </p>
            </div>
          )}
