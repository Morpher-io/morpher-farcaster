  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-lg font-bold">Close Position</CardTitle>
      ...
      <CardContent>
        ...
        <Button
          ...
          variant={
            selectedPosition?.direction !== "long" ? "secondary" : "default"
          }
        >
          {tradeExecuting && <Loader2Icon className="animate-spin" />}
          <span className="text-white">Close {selectedPosition?.symbol}</span>
        </Button>

        {tradeError && (
          <div className="mt-2 text-sm text-red-500">{tradeError}</div>
        )}
      </CardContent>
    </Card>
  );
