...
import MorpherTradeSDK, { tokenValueFormatter, usdFormatter  } from "../../../../morpher-trading-sdk/src/index";
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex items-center text-2xl font-bold">
          <Input type="number" placeholder="0.00" className="h-auto flex-1 p-2 text-2xl font-bold focus-visible:ring-0 focus-visible:ring-offset-0" value={tradeAmount} onChange={(e) => setTradeAmount(e.target.value)} />
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <Button variant="ghost" role="combobox" aria-expanded={open} className="w-auto justify-between">
                {selectedCurrency && (
                  <img
                    ...
                    {currencyList && Object.entries(currencyList).map(([currency, details]) => (
                      ...
                      >
                        ...
                        <div className="ml-auto text-right text-sm">
                          ...
                          <div className="text-xs text-muted-foreground">${(details.usd || 0).toFixed(2)}</div>
                        </div>
                      </CommandItem>
                    ))}
                  </CommandGroup>
