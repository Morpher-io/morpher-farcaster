  ...
  const openPosition = () => {
    try {
      ...
      if (tradeAmount && Number(tradeAmount) > 0 && currencyDetails) {
        tradeAmountFormatted = BigInt(
          Math.round(
            Number(tradeAmount) * 10 ** (currencyDetails.decimals || 18),
          ),
        );
      }
      ...
      morpherTradeSDK.openPosition({
        ...
        tradeAmount: tradeAmountFormatted,
        callback: tradeComplete,
      });
    } catch (err: any) {
    ...
    <div className="mt-4 mb-4">
      {loading ? (
        ...
        <div className="">
              ...
              {currencyList &&
                Object.entries(currencyList).map(([currency, details]) => (
                  <TabsTrigger
                    ...
                    value={currency}
                    disabled={
                      !details.balance || BigInt(details.balance) === 0n
                    }
                    className={
                      "h-auto flex-col " +
                      (currency == selectedCurrency ? "border-primary" : "")
                    }
                  >
                    <img
          ...
          <div className="space-y-2">
            <div className="bg-muted rounded-lg rounded-t-none border border-t-0 p-3">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <Input
                    ...
                    className="placeholder:text-primary h-auto w-full border-none bg-transparent p-0 text-2xl font-bold placeholder:opacity-60 focus-visible:ring-0 focus-visible:ring-offset-0"
                  />
                </div>
                {selectedCurrency !== "USDC" && (
                  ...
                  >
                    <ToggleGroupItem
                      ...
                      className="data-[state=on]:bg-primary/20 p-1.5"
                    >
                      <img
              ...
              <div className="grid grid-cols-5 gap-2 py-2">
                {[
                  ...
                  const currentUsdValue =
                    (Number(tradeAmount) || 0) * exchangeRate;
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-8 rounded-sm bg-[var(--blue)] text-xs font-bold text-white transition-opacity hover:bg-[var(--blue)] hover:text-white hover:opacity-70"
              ...
              <div className="text-muted-foreground mt-1 flex justify-between px-1 text-xs">
                ...
                <span>
                  ...
                  {selectedCurrency !== "USDC" &&
                    ` ($${usdFormatter(maxBalance * exchangeRate)})`}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        <ToggleGroup
          type="single"
          value={tradeType}
          className="grid w-[100px] grid-cols-2 rounded-lg border"
      ...
      <div className="text-muted-foreground bg-muted mb-4 space-y-2 rounded-md p-3 text-xs">
        {tradeType === "long" ? (
          <div className="flex items-start gap-2">
            <TrendingUp className="text-primary mt-0.5 h-4 w-4 shrink-0" />
            <p>
              {t("LONG_DESCRIPTION", {
                market: selectedMarket?.symbol || "the asset",
              })}
            </p>
          </div>
        ) : (
          <div className="flex items-start gap-2">
            <TrendingDown className="text-secondary mt-0.5 h-4 w-4 shrink-0" />
            <p>
              {t("SHORT_DESCRIPTION", {
                market: selectedMarket?.symbol || "the asset",
              })}
      ...
      <div className="mb-6">
        ...
        {leverage[0] > 1 && marketData && (
          <LeverageImpactVisualizer
            ...
            tradeType={tradeType}
            marketPrice={
              livePrice && livePrice[marketData.market_id]
                ? livePrice[marketData.market_id]
                : Number(marketData.close) || 0
            }
      ...
      >
        {tradeExecuting && <Loader2Icon className="mr-2 animate-spin" />}
      ...
      {(!tradeAmount || Number(tradeAmount) <= 0) && (
        <p
          className="mt-3 flex cursor-pointer items-center justify-center text-[var(--info)]"
          onClick={() => {
            window.scrollTo(0, 0);
          }}
        >
