...
export const MarketChart = React.memo(function MarketChart({
  ...
  const chartData = React.useMemo(() => {
    if (timeRange == "1D") {
      return data?.map((d) => {
        return {
          timestamp: new Date(Number(d[0]) * 1000).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
          close: d[4],
      ...
      return data?.map((d) => ({
        timestamp: new Date(Number(d[0]) * 1000).toLocaleDateString([], {
          day: "2-digit",
          month: "2-digit",
        }),
        close: d[4],
  ...
  if (!chartData || chartData.length === 0) {
    return <Skeleton className="mt-4 h-[200px] w-full" />;
  }
