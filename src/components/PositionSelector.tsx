...
export function PositionSelector() {
      ...
      <div className="flex w-full items-center justify-between">
        <div className="flex items-center">
          <div>
            {position.logo_image && (
              <img
                ...
                className="ml--2 mr-4 h-8 w-8 rounded-lg"
              />
            )}
          </div>
          <div
          ...
          >
            ...
            <p className="truncate font-normal">{position?.name}</p>
          </div>
        </div>
        <div id="marketValue" className="flex flex-col text-right">
          <p className="text-base font-bold">
            {currencyList?.MPH?.usd_exchange_rate
              ? "$ " +
                usdFormatter(
                  (Number(position.value || 0) / 10 ** 18) *
                    currencyList?.MPH?.usd_exchange_rate,
                )
              : tokenValueFormatter(Number(position.value || 0) / 10 ** 18) +
                " MPH"}
          </p>
          <div
            id="marketPercent"
          ...
          >
            ...
            {Number(position?.total_return_percent || 0) > 0 ? "+" : ""}
            {Number(Number(position?.total_return_percent || 0) * 100).toFixed(
              2,
            )}{" "}
            %
          </div>
  ...
  const fetchPositions = async () => {
    ...
    let positionList = await morpherTradeSDK.getPositions({
      eth_address: address,
    });
